...
import { outbound } from '../../services/stockApi'
import './index.scss'
...
export default function OutboundPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!Number.isInteger(qty) || qty <= 0) {
      Taro.showToast({ title: '出库数量必须为正整数', icon: 'none' })
      return
    }
    setLoading(true)
    try {
      const res = await outbound(currentShop.id, { productId: product.id, quantity: qty })
      setUpdatedQty(res.data.updatedInventory.quantity)
      Taro.showToast({ title: '出库成功', icon: 'success' })
      setProduct(null)
      setQuantity('')
    } catch {
      // handled in request (including insufficient stock message)
    } finally {
      setLoading(false)
    }
  }
    ...
    <View className='outbound-page'>
      ...
      {product && (
        <View className='product-card'>
          ...
          <View className='qty-input-row'>
            <Text className='label'>出库数量</Text>
            <Input
              className='qty-input'
              type='number'
              value={quantity}
      ...
      {updatedQty !== null && (
        <View className='result-card'>
          <Text className='result-text'>✅ 出库成功</Text>
